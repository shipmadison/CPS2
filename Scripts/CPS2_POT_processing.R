...
# LOAD DATA -----------------------------------------------------------------------------------------------------------------
  ...
  # Load summary catch and specimen tables
      catch <- list.files("./Data/Pot Data/Catch - FTP/") %>%
        purrr::map_df(~read.csv(paste0("./Data/Catch - FTP/", .x))) 
      
      specimen <- list.files("./Data/Pot Data/Specimen - FTP/") %>%
        purrr::map_df(~read.csv(paste0("./Data/Specimen - FTP/", .x))%>% mutate(STATION = paste0("X", STATION))) 
      
  # Load raw data for processing below   
      raw_sample <- list.files("./Data/Pot Data/Raw Data - FTP/", pattern = "_SAMPLE_0") %>% # RECORDS of SAMPLE INFO
        purrr::map_df(~read.csv(paste0("./Data/Raw Data - FTP/", .x))) #E.G. SEX, SPECIES
      
      raw_sample_values <- list.files("./Data/Pot Data/Raw Data - FTP/", pattern = "_SAMPLE_VALUES") %>% #RECORDS OF # TOSSED
        purrr::map_df(~read.csv(paste0("./Data/Pot Data/Raw Data - FTP/", .x))) %>%
        mutate(TOSSED = ifelse(is.na(COUNT) == FALSE, COUNT,0)) %>%
        group_by(HAUL_ID, CATCH_SAMPLE_ID) %>%
        dplyr::reframe(TOSSED = sum(TOSSED)) 
      
      raw_specimen <- list.files("./Data/Pot Data/Raw Data - FTP/", pattern = "_SPECIMEN_0") %>% 
        purrr::map_df(~read.csv(paste0("./Data/Raw Data - FTP/", .x))) %>%
        dplyr::select(HAUL_ID, SPECIMEN_ID, CATCH_SAMPLE_ID, SPECIES_CODE)
      
      raw_specimen_bio<- list.files("./Data/Pot Data/Raw Data - FTP/", pattern = "_SPECIMEN_BIOMETRICS") %>% 
        purrr::map_df(~read.csv(paste0("./Data/Pot Data/Raw Data - FTP/", .x))) %>%
